...
```bash
curl -fsSL -o upgrade_preflight.sh \
  "https://raw.githubusercontent.com/sctsivali/orcastra-dashboard/${RELEASE}/scripts/upgrade_preflight.sh"
chmod +x upgrade_preflight.sh
...
./upgrade_preflight.sh --to "${VERSION}" --new-compose ./docker-compose.prod.yml.new
```

It exits non-zero on a blocking finding and zero on advisories. Read the advisories anyway.
...
# run the preflight again: with the target image now local, it reads the expected schema
# head out of the image itself rather than from its own constant
./upgrade_preflight.sh --to "${VERSION}" --new-compose ./docker-compose.prod.yml
...
```bash
curl -fsSL -o rollback_release.sh \
  "https://raw.githubusercontent.com/sctsivali/orcastra-dashboard/${RELEASE}/scripts/rollback_release.sh"
chmod +x rollback_release.sh
./rollback_release.sh --to <previous-version>
```

Three things the script handles that a manual `pull` and `up -d` does not.
...
is the escape hatch when the migrations in between are forward-compatible.
